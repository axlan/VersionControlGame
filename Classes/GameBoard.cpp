...
Point2D GameBoard::BoardPixel2Point(int x, int y) const
{
	...
	int point_x = int(x / tile_size.x);
	int point_y = int(y / tile_size.y);
...
void GameBoard::on_mouse_down(cocos2d::Event* event)
{
	...
	CCLOG("tile X=%d Y=%d", point.x, point.y);

	bool done = true;

	for (auto &tile : _merge_tiles) {
		if (point == tile.position) {
	...
	if (done) {
		GraphNode* graph_node = (_graph_node)?_graph_node : _merge_pair->_graph_node;
		GameState game_state;
		CopyLayerState(_dynamic, game_state.dynamic_layer);
		...
		for (const auto &tile : _merge_tiles) {
			...
			if (chosen->gid == 0) {
				...
				if (chosen != &tile) {
					int map_id = IntFromPoint(getTileCoord(tile.position), _dynamic);
					game_state.dynamic_layer.erase(map_id);
				}
				int map_id = IntFromPoint(getTileCoord(chosen->position), _dynamic);
...
{
	Color4F color = (_graph_node) ? Color4F::MAGENTA : Color4F::BLACK;
	DrawBounds(color);
	_merge_tiles.clear();
	_merging = false;
}
...
void GameBoard::DrawMergeTile(const MergeTile &tile)
{
	...
	Point tile_size = GetTileSize();
	Point origin(tile.position.x * tile_size.x, tile.position.y * tile_size.y);
	boundLines->drawRect(origin, origin + Point(tile_size.x, tile_size.y), color);
}
